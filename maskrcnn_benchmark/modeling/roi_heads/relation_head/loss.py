...
class RelationLossComputation(object):
    ...
    def __init__(
        ...
        if self.use_label_smoothing:
            ...
            # ============================================================
            # CB-Loss Reweighting for Relation Classification (L_e_sim)
            # Only applied when cb_loss_beta is specified
            # Reference: Cui et al., "Class-Balanced Loss Based on Effective
            # Number of Samples", CVPR 2019
            # ============================================================
            if cb_loss_beta is not None:
                beta = cb_loss_beta
                # predicate_proportion contains 50 foreground predicate proportions
                # (classes 1-50), summing to ~1.0
                # Convert to estimated sample counts using a reference total
                # The exact total doesn't critically matter because the CB formula's
                # relative weighting is robust to scaling when beta is close to 1
                total_fg_samples = 287000  # approximate VG150 foreground relation count
                fg_counts = np.array(predicate_proportion) * total_fg_samples
                fg_counts = np.maximum(fg_counts, 1.0)
                
                # Background class (class 0) count estimation:
                # With POSITIVE_FRACTION=0.25, background samples are ~3x foreground
                bg_count = total_fg_samples * 3.0
                
                # All 51 classes: [background, pred_1, pred_2, ..., pred_50]
                samples_per_cls = np.concatenate([[bg_count], fg_counts])
                
                num_classes = len(samples_per_cls)  # 51
                ...
                print(f"[CB-Loss] Head pred 'on' (cls 31) weight: {cb_weights[31]:.6f}")
                print(f"[CB-Loss] Applied ONLY to relation classification loss (L_e_sim)")
                print("=" * 60)
            else:
                self.criterion_loss = nn.CrossEntropyLoss()
        
        # Object classification loss: ALWAYS unweighted
        # This is critical: CB-Loss must NOT affect object classification
        if self.use_label_smoothing:
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits):
        ...
        # Relation loss: uses CB-weighted CrossEntropyLoss (if reweighting is enabled)
        loss_relation = self.criterion_loss(relation_logits, rel_labels.long())
        # Object loss: ALWAYS uses unweighted CrossEntropyLoss
